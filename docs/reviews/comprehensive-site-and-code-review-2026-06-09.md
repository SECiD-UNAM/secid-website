...
A second wave (`e63e114` + two follow-ups, 2026-06-13) cleared four more
of the deferred items: the hydration date-mismatch root cause (shared
timezone-pinned `formatDate` helper), the `JobMatches`/`UpcomingEvents`
universal-listing migration, and the `security-config.ts` `@ts-nocheck`
removal (which uncovered and fixed a latent captcha-provider wiring bug).
The `onUserDelete` v1→v2 item was found **not actionable** — Firebase
Functions v2 has no user-deletion trigger (`beforeUserDeleted` does not
exist; only `beforeUserCreated`/`beforeSignIn`/email/sms), so the existing
v1 `auth.user().onDelete` is the correct API and was left in place.
...
| ~~`JobMatches`/`UpcomingEvents` bypass the universal listing~~ **DONE** | `src/components/dashboard/`                                                                                         | Migrated to `useUniversalListing` + adapters (2026-06-13).                                                                                                                                                                                                                          |
| ~~React hydration errors #418/#423/#425~~ **DONE**                      | SSR-time static/prop dates                                                                                          | Root cause was timezone-unpinned `toLocaleDateString` on SSR-available dates; fixed with a timezone-pinned shared `formatDate` (2026-06-13). The logged-in-CTA swap is a separate UX task (AuthContext starts `loading`, so it's not a hydration error — the CTAs just never swap). |
| ~~`onUserDelete` on Functions v1~~ **NOT ACTIONABLE**                   | `functions/src/index.ts:217-238`                                                                                    | Functions v2 has no user-deletion trigger; v1 `onDelete` is the correct API. Left as-is.                                                                                                                                                                                            |
| ~~`@ts-nocheck` in `security-config.ts`~~ **DONE**                      | `src/lib/security-config.ts:1`                                                                                      | Pragma removed; fixed a latent captcha-provider wiring bug in the process (2026-06-13).                                                                                                                                                                                             |
