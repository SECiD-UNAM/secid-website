    ...
    content = content.replace(
      ...
      (match, quote1, prefix, quote2, key, suffix, rest) => {
        ...
        const cleanSuffix = suffix
          ...
          .replace(/^\./, '');
        const fullKey =
          cleanPrefix + key + (cleanSuffix ? '.' + cleanSuffix : '');
    ...
    content = content.replace(
      ...
      (match, outerQuote, prefix, innerQuote, key, suffix) => {
        ...
        if (
          ...
          prefix.startsWith('forum') ||
          prefix.startsWith('analytics') ||
          prefix.startsWith('assessment')
        ) {
          fixCount++;
    ...
    if (fixCount > 0 && content.includes('getTranslation(')) {
      if (!content.includes('import { getTranslation')) {
        ...
        const hasTranslationsImport =
          /import .* from ['"]@\/lib\/translations['"]/.test(content);
