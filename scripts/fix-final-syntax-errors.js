...
const filesToFix = [
  {
    ...
    fixes: [
      {
        line: 95,
        find: '{ entryTypes: .largest-contentful-paint }',
        replace: "{ entryTypes: ['largest-contentful-paint'] }",
      },
      {
        line: 108,
        find: '{ entryTypes: .paint }',
        replace: "{ entryTypes: ['paint'] }",
      },
      {
        line: 152,
        find: '{ entryTypes: .first-input, .event }',
        replace: "{ entryTypes: ['first-input', 'event'] }",
      },
      {
        line: 207,
        find: '{ entryTypes: .layout-shift }',
        replace: "{ entryTypes: ['layout-shift'] }",
      },
      {
        line: 251,
        find: '{ entryTypes: .longtask }',
        replace: "{ entryTypes: ['longtask'] }",
      },
      {
        line: 272,
        find: '{ entryTypes: .navigation }',
        replace: "{ entryTypes: ['navigation'] }",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        line: 366,
        find: /document\[.removeEventListener.\]\('visibilitychange'/,
        replace: "document.removeEventListener('visibilitychange'",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        find: /customer\[.metadata.\]\.userId/,
        replace: "customer['metadata']?.userId",
      },
      {
        find: /payment_intent\[.metadata.\]/,
        replace: "payment_intent['metadata']",
      },
    ],
  },
  {
    file: 'src/lib/security-config.ts',
    fixes: [{ find: /request\[.ip.\]/, replace: "request['ip']" }],
  },
  {
  ...
  },
  {
    ...
    fixes: [{ find: /\[.script.\]/, replace: "['script']" }],
  },
  {
  ...
  },
  {
    ...
    fixes: [
      {
        find: "const keys = path.split('');",
        replace: "const keys = path.split('.');",
      },
    ],
  },
];
