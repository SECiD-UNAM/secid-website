...
async function main() {
  ...
  console.log(
    `Current profile.companyId: ${userData.profile?.companyId || 'none'}`
  );
  console.log(
    `Current profile.company: ${userData.profile?.company || 'none'}`
  );
  ...
  for (const r of roles) {
    console.log(
      `  - ${r.company} | companyId: ${r.companyId || 'MISSING'} | current: ${r.current}`
    );
  }
  ...
  allCompanies.docs.forEach((d) =>
    companyMap.set(d.data().name?.toLowerCase(), { id: d.id, ...d.data() })
  );
  ...
  for (const role of roles) {
    if (role.company && !role.companyId) {
      ...
      if (existing) {
        ...
        console.log(
          `  Created "${role.company}" → ${newRef.id} (pending review)`
        );
  ...
  if (oldCompanyId && oldCompanyId !== newCompanyId) {
    try {
      ...
      const oldName = allCompanies.docs
        .find((d) => d.id === oldCompanyId)
        ?.data()?.name;
  ...
  if (newCompanyId && newCompanyId !== oldCompanyId) {
    try {
      ...
      const newName =
        companyMap.get(currentRole?.company?.toLowerCase())?.name ||
        newCompanyId;
