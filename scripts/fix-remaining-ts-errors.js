  ...
  for (const file of files) {
    ...
    for (const importLine of importLines) {
      ...
      const match = importLine.match(
        /import\s+(?:type\s+)?{([^}]+)}|import\s+(?:type\s+)?(\w+)|import\s+\*\s+as\s+(\w+)/
      );
      if (match) {
        ...
        if (imports) {
          ...
          if (
            isUsed ||
            importLine.includes('React') ||
            importLine.includes('firebase')
          ) {
            usedImports.push(importLine);
          } else {
            fixCount++;
            console.log(
              `  Removing unused import: ${importNames.join(', ')} from ${file}`
            );
          }
        } else {
          usedImports.push(importLine);
    ...
    content = content.replace(
      ...
      (match, obj, prop1, prop2) => {
        ...
        if (
          obj.match(/^(data|result|response|user|profile|settings|config)$/)
        ) {
          fixCount++;
