...
const commands = {
  ...
  'find-large': {
    ...
    run: () => {
      ...
      if (largeFiles.length > 0) {
        console.log(
          `Files larger than ${(maxSize / 1024 / 1024).toFixed(2)}MB:\n`
        );
        largeFiles
          ...
          .forEach(({ path: filePath, size }) => {
            ...
            console.log(
              `${colors.yellow}${(size / 1024 / 1024).toFixed(2)}MB${colors.reset}  ${relPath}`
            );
          });
      } else {
  ...
  'project-stats': {
    ...
    run: () => {
      ...
      console.log(
        `  Code: ${stats.lines.code.toLocaleString()} (${((stats.lines.code / stats.lines.total) * 100).toFixed(1)}%)`
      );
      console.log(
        `  Comments: ${stats.lines.comments.toLocaleString()} (${((stats.lines.comments / stats.lines.total) * 100).toFixed(1)}%)`
      );
      console.log(
        `  Blank: ${stats.lines.blank.toLocaleString()} (${((stats.lines.blank / stats.lines.total) * 100).toFixed(1)}%)`
      );
    },
  },
...
function main() {
  ...
  if (!command || command === 'help') {
    ...
    Object.entries(commands).forEach(([cmd, { description }]) => {
      console.log(
        `  ${colors.yellow}${cmd.padEnd(15)}${colors.reset} ${description}`
      );
    });
