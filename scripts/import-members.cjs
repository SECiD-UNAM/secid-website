...
async function importMembers() {
  ...
  console.log(
    `Loaded ${members.length} records (${data.memberCount} members, ${data.collaboratorCount} collaborators)`
  );
  ...
  for (const member of members) {
    ...
    try {
      if (isDryRun) {
        ...
        console.log(
          `    role=${member.role} lifecycle=${member.lifecycleStatus} cuenta=${member.numeroCuenta || 'N/A'} completeness=${doc.profileCompleteness}%`
        );
        created++;
        continue;
      }
      ...
      if (existingDoc.exists) {
        console.log(`  SKIP  ${label} — complete (uid: ${uid})`);
        skipped++;
        continue;
      }
      ...
      console.log(
        `  OK    ${label} -> uid: ${uid} (${member.role}/${member.lifecycleStatus})${authNote}`
      );

      // 3. Generate password reset link only for test email
      if (
        member.email.toLowerCase() === TEST_EMAIL.toLowerCase() &&
        !resetSent
      ) {
        try {
          ...
          console.log(
            `  WARN  Could not generate reset link for ${member.email}: ${e.message}`
          );
        }
      }
