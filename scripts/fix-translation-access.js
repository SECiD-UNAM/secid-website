...
async function fixTranslationAccess() {
  ...
  for (const file of files) {
    ...
    const translationArrayAccess =
      /translations\[(lang|language|currentLang|selectedLang|userLang)\]/g;
    ...
    if (genericArrayAccess.test(content)) {
      content = content.replace(genericArrayAccess, (match, varName) => {
        ...
        if (
          varName.toLowerCase().includes('lang') ||
    ...
    if (fixCount > 0) {
      ...
      if (
        content.includes('getTranslation(') &&
        !content.includes('import { getTranslation')
      ) {
      ...
      if (
        content.includes('as Language') &&
        !content.includes('import type { Language')
      ) {
      ...
      if (
        content.includes('safeTranslationAccess(') &&
        !content.includes('import { safeTranslationAccess')
      ) {
        const importRegex = /import .* from ['"]@\/lib\/translations['"]/;
        if (importRegex.test(content)) {
          // Add to existing import
