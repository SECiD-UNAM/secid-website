...
const memberDirPath = path.join(
  __dirname,
  '../src/components/directory/MemberDirectory.tsx'
);
...
const subManagerPath = path.join(
  __dirname,
...
if (fs.existsSync(messagingPath)) {
  ...
  const propertyAccessPatterns = [
    {
      pattern: /'metadata\['unreadCount'\]'/,
      replacement: 'metadata: { unreadCount',
    },
    { pattern: /'metadata\['readBy'\]'/, replacement: 'metadata: { readBy' },
    {
      pattern: /'metadata\['deletedBy'\]'/,
      replacement: 'metadata: { deletedBy',
    },
    {
      pattern: /'metadata\['archivedBy'\]'/,
      replacement: 'metadata: { archivedBy',
    },
    {
      pattern: /'metadata\['participants'\]'/,
      replacement: 'metadata: { participants',
    },
    {
      pattern: /'metadata\['lastMessage'\]'/,
      replacement: 'metadata: { lastMessage',
    },
    {
      pattern: /'metadata\['typingUsers'\]'/,
      replacement: 'metadata: { typingUsers',
    },
  ];
