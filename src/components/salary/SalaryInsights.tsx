...
export function SalaryInsights({ lang = 'es' }: Props) {
  ...
  const t = {
    ...
    byIndustry: lang === 'es' ? 'Salario por Industria' : 'Salary by Industry',
    benefits: lang === 'es' ? 'Beneficios mas Comunes' : 'Most Common Benefits',
    breakdown:
    ...
    memberCta: lang === 'es' ? 'Agregar mis datos' : 'Add my data',
    publicCta: lang === 'es' ? 'Iniciar sesion' : 'Sign in',
    contributorBadge:
      lang === 'es'
        ? 'Acceso completo — gracias por contribuir'
        : 'Full access — thanks for contributing',
    memberBadge:
      lang === 'es'
        ? 'Vista limitada — comparte tus datos para ver todo'
        : 'Limited view — share your data to see everything',
    privacy:
      lang === 'es'
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-6 text-center dark:border-blue-800 dark:bg-blue-900/20">
          ...
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"
            />
          </svg>
      ...
      {stats?.distribution && stats.distribution.length > 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
          <h3 className="mb-4 text-sm font-medium text-gray-700 dark:text-gray-300">
            {lang === 'es'
              ? 'Distribucion Salarial (Mensual Bruto)'
              : 'Salary Distribution (Monthly Gross)'}
          </h3>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
                  {lang === 'es'
                    ? `${stats.rawData.length} registros individuales disponibles`
                    : `${stats.rawData.length} individual records available`}
                </p>
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  {lang === 'es'
                    ? 'Acceso admin — datos con información personal'
                    : 'Admin access — data with personal information'}
                </p>
              </div>
            </div>
            <a
