...
function buildSummary(comp: Compensation, lang: 'es' | 'en'): string {
  if (!comp.monthlyGross)
    return lang === 'es' ? 'Compensación registrada' : 'Compensation recorded';
...
export const CompensationFields: React.FC<Props> = ({
    ...
    (async () => {
      try {
        const snap = await getDoc(
          doc(db, 'users', userId, 'compensation', roleId)
        );
  ...
  const handleCountryChange = (country: 'MX' | 'US' | 'OTHER') => {
    ...
    const fiscalRegime =
      country === 'MX' ? 'asalariado' : country === 'US' ? 'w2' : undefined;
        ...
        <div className="space-y-5 border-t border-gray-200 px-4 pb-5 pt-4 dark:border-gray-700">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleCountryChange(e.target.value as 'MX' | 'US' | 'OTHER')
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  update({ currency: e.target.value as 'MXN' | 'USD' })
                }
                className={SELECT_CLASS}
              >
          ...
          {current.country !== 'OTHER' && (
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  update({
                    fiscalRegime: e.target
                      .value as Compensation['fiscalRegime'],
                  })
                }
                className={SELECT_CLASS}
              >
          ...
          {netResult && (
            <div className="rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                {lang === 'es'
                  ? 'Cálculo estimado (neto)'
                  : 'Estimated calculation (net)'}
              </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  update({
                    annualBonusType: e.target.value as 'fixed' | 'percentage',
                  })
                }
                className={SELECT_CLASS}
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                {lang === 'es'
                  ? 'Stock / RSU (valor anual)'
                  : 'Stock / RSU (annual value)'}{' '}
                <span className="font-normal text-gray-500">
                  ({lang === 'es' ? 'opcional' : 'optional'})
                </span>
              </label>
          ...
          {totalComp !== null && current.monthlyGross && (
            <div className="rounded-lg border border-primary-200 bg-primary-50 p-3 dark:border-primary-800 dark:bg-primary-900/20">
              <p className="text-xs text-primary-700 dark:text-primary-300">
                {lang === 'es'
                  ? 'Compensación total estimada (anual)'
