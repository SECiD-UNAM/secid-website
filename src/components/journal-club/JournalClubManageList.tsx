...
const labels: Record<string, Record<string, string>> = {
  title: { en: 'Journal Club Management', es: 'Gestion del Journal Club' },
  newSession: { en: 'New Session', es: 'Nueva sesion' },
  searchPlaceholder: {
    en: 'Search by topic or presenter...',
    es: 'Buscar por tema o presentador...',
  },
  ...
  confirmDelete: {
    ...
    es: 'Estas seguro de que deseas eliminar esta sesion?',
  },
  noSessions: {
    en: 'No journal club sessions found.',
    es: 'No se encontraron sesiones del journal club.',
  },
  loading: { en: 'Loading sessions...', es: 'Cargando sesiones...' },
  errorLoading: {
    en: 'Error loading sessions. Please try again.',
    es: 'Error al cargar sesiones. Intenta de nuevo.',
  },
  sessionCount: { en: 'sessions', es: 'sesiones' },
};
...
interface JournalClubManageListProps {
  lang?: 'es' | 'en';
}

export default function JournalClubManageList({
  lang = 'es',
}: JournalClubManageListProps) {
  const { user } = useAuth();
  ...
  const canCreate = can('journal-club', 'create');
  const canEdit = can('journal-club', 'edit');
  const canDelete = can('journal-club', 'delete');
  const canPublish = can('journal-club', 'publish');
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await getAllJournalClubSessions();
  ...
  const handleDelete = async (session: JournalClubSession) => {
    if (!confirm(t('confirmDelete', lang))) return;
    try {
      await deleteJournalClubSession(session.id);
  ...
  const handleTogglePublish = async (session: JournalClubSession) => {
    if (!user) return;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canCreate && (
          <a
            href={`/${lang}/dashboard/journal-club/new`}
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        {filtered.length} {t('sessionCount', lang)}
      </p>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {canPublish && session.status !== 'cancelled' && (
                        <button
                          type="button"
                      ...
                      {canEdit && (
                        <a
                          href={`/${lang}/dashboard/journal-club/edit/${session.id}`}
                          title={t('edit', lang)}
