  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const sessionData: Partial<MentorshipSession> = {
        ...
        resources:
          formData.resources.length > 0 ? formData.resources : undefined,
        ...
        <form onSubmit={handleSubmit} className="session-form-content">
          ...
          <section className="form-section">
            ...
            <div className="form-group">
              <label htmlFor="title">{t.mentorship.sessions.title} *</label>
              <input
                type="text"
                id="title"
                ...
                {formData.resources.map((resource, index) => (
                  <div key={index} className="resource-item">
                    <div className="resource-info">
                      <i
                        className={`fas fa-${RESOURCE_TYPES.find((t) => t.value === resource.type)?.icon}`}
                      ></i>
    ...
    <div className="mentorship-sessions">
      ...
      <div className="sessions-controls">
        ...
        <div className="view-stats">
          <span>
            {t.mentorship.sessions.showing} {getFilteredSessions().length}{' '}
            ...
            <div key={session.id} className={`session-card ${session.status}`}>
              ...
              <div className="session-details">
                ...
                <div className="session-type">
                  <i
                    className={`fas fa-${SESSION_TYPES.find((t) => t.value === session['type'])?.icon}`}
                  ></i>
                  <span>
                  ...
                  <div className="session-homework">
                    ...
                    <div className="homework-preview">
                      {session.homework.slice(0, 2).map((hw, index) => (
                        <div
                          key={index}
                          className={`homework-item ${hw.completed ? 'completed' : ''}`}
                        >
                          <i
                            className={`fas fa-${hw.completed ? 'check-circle' : 'circle'}`}
                          ></i>
                          <span>{hw.title}</span>
                        </div>
                      ))}
                      {session.homework.length > 2 && (
                        <span className="homework-more">
                          +{session.homework.length - 2}{' '}
                          {t.mentorship.sessions.more}
                        </span>
                      )}
