...
const LearningPaths: React.FC<LearningPathsProps> = ({ currentUser }) => {
  ...
  const [view, setView] = useState<'paths' | 'courses' | 'my-learning'>(
    'paths'
  );
  ...
  const categories: { value: CourseCategory | 'all'; label: string }[] = [
    ...
    { value: 'data-engineering', label: t('learning.dataEngineering') },
    {
      ...
      label: t('learning.businessIntelligence'),
    },
    ...
    { value: 'cloud-computing', label: t('learning.cloudComputing') },
    {
      ...
      label: t('learning.dataScienceFundamentals'),
    },
  ];
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [pathsData, coursesData, enrollmentsData, recommendedData] =
        await Promise.all([
          getLearningPaths(),
          getCourses(),
  ...
  const myEnrolledCourses = enrollments
    .map((enrollment) =>
  ...
  const formatDuration = (hours: number) => {
    if (hours < 1)
      return t('learning.minutesDuration', { minutes: Math.round(hours * 60) });
    if (hours < 24)
      return t('learning.hoursDuration', { hours: Math.round(hours) });
  ...
  const renderPathCard = (path: LearningPath) => (
    ...
    >
      ...
      <div className="p-6">
        ...
        <p className="mb-4 line-clamp-3 text-gray-600">{path['description']}</p>

        <div className="mb-4 flex items-center justify-between text-sm text-gray-500">
          <span>
            {t('learning.coursesCount', { count: path.courses.length })}
          </span>
    ...
    return (
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="mb-4 flex items-center justify-between text-sm text-gray-500">
            <span>
              {t('learning.lessonsCount', { count: course.lessons.length })}
            </span>
            ...
            <div className="flex items-center">
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`h-4 w-4 ${i < Math.floor(course.averageRating) ? 'text-yellow-400' : 'text-gray-300'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
          ...
          <div className="flex space-x-2">
            ...
            <button
              onClick={() =>
                (window.location.href = `/learning/courses/${course.id}/preview`)
              }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-lg text-gray-600">{t('learning.subtitle')}</p>
      </div>
