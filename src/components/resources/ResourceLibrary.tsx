      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {categories.map(([category, count]) => (
          ...
          >
            ...
            <div className="text-xs text-gray-500">{count} {t?.resources?.resources || 'resources'}</div>
          </button>
        ))}
      </div>
    );
      ...
      <div className="bg-blue-50 rounded-lg p-4 mb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            ...
            <div className="text-sm text-gray-600">{t?.resources?.totalResources || 'Total Resources'}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-blue-600">{stats.totalDownloads}</div>
            <div className="text-sm text-gray-600">{t?.resources?.totalDownloads || 'Downloads'}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-blue-600">{stats.totalAuthors}</div>
            <div className="text-sm text-gray-600">{t?.resources?.contributors || 'Contributors'}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-blue-600">{stats.averageRating.toFixed(1)}</div>
            <div className="text-sm text-gray-600">{t?.resources?.averageRating || 'Avg Rating'}</div>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex bg-gray-100 rounded-lg p-1">
          <button
            ...
            className={`p-2 rounded-md ${
              activeView === 'grid' ? 'bg-white shadow-sm' : 'text-gray-600'
            }`}
          >
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">
          {t?.resources?.library || 'Resource Library'}
        </h1>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          {t?.resources?.libraryDescription || 'Discover and share learning materials, templates, tools, and datasets to accelerate your data science journey.'}
        </p>
      </div>
