...
export default function ResourceDetail({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (resource.accessLevel === 'premium' && !user) {
        alert(
          t?.resources?.loginRequired ||
            'Please login to download premium resources'
        );
        return;
      }
  ...
  const handleBookmark = async () => {
    if (!user) {
      alert(
        t?.resources?.loginRequired || 'Please login to bookmark resources'
      );
      return;
    }
    ...
    try {
      if (isBookmarked) {
        ...
        setIsBookmarked(false);
        setResource((prev) =>
          prev ? { ...prev, bookmarkCount: prev.bookmarkCount - 1 } : null
        );
      } else {
  ...
  const renderStars = (
    ...
    return Array.from({ length: 5 }, (_, i) => (
      <button
        ...
        className={`text-lg ${interactive ? 'cursor-pointer hover:text-yellow-400' : ''} ${
          i < Math.floor(rating)
            ? 'text-yellow-400'
    ...
    return (
      <div className="py-8 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          {t?.resources?.notFoundDescription ||
          ...
          <div className="flex items-center gap-6 text-sm text-gray-600 dark:text-gray-400">
            <div className="flex items-center gap-1">
              ...
              <span>
                ...
                {t?.resources?.downloads || 'downloads'}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <span>👁️</span>
              <span>
                {resource.viewCount} {t?.resources?.views || 'views'}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <span>❤️</span>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {resource.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="rounded-full bg-blue-50 px-3 py-1 text-sm text-blue-700 dark:bg-blue-900/20 dark:text-blue-400"
                      >
                        #{tag}
                      </span>
                    ))}
      ...
      {relatedResources.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-4 text-xl font-semibold">
            {t?.resources?.related || 'Related Resources'}
          </h2>
