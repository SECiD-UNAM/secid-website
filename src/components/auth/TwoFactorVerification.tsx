...
const backupCodeSchema = z.object({
  backupCode: z
    .string()
    .min(8, 'Backup code must be 8 digits')
    .max(8, 'Backup code must be 8 digits')
    .regex(/^\d+$/, 'Backup code must contain only numbers'),
});
...
export const TwoFactorVerification: React.FC<TwoFactorVerificationProps> = ({
  ...
  const handleVerifyCode = async (data: VerificationFormData) => {
    if (!uid && !sessionId) {
      toast['error'](
        lang === 'es'
          ? 'Información de sesión faltante'
    ...
    try {
      ...
      if (isValid) {
        toast.success(
          lang === 'es' ? 'Verificación exitosa' : 'Verification successful'
        );
        onSuccess?.();
      } else {
      ...
      toast.error(
        error['message'] ||
          (lang === 'es' ? 'Error de verificación' : 'Verification error')
      );
      setAttemptsLeft((prev) => prev - 1);
    } finally {
      setLoading(false);
  ...
  const handleUseBackupCode = async (data: BackupCodeFormData) => {
    if (!uid) {
      toast['error'](
        lang === 'es' ? 'ID de usuario faltante' : 'Missing user ID'
      );
      return;
    }
    ...
    try {
      ...
      if (isValid) {
        toast.success(
          lang === 'es' ? 'Código de respaldo válido' : 'Valid backup code'
        );
        onSuccess?.();
      } else {
        toast.error(
      ...
      toast['error'](
        error['message'] ||
          (lang === 'es' ? 'Error con código de respaldo' : 'Backup code error')
      );
    } finally {
      setLoading(false);
      resetBackup();
    ...
    <div className="mx-auto max-w-md">
      <div className="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
        ...
        {!showBackupCode ? (
          ...
          >
            ...
            <div className="text-center">
              <button
                ...
                className="text-sm text-primary-600 underline hover:text-primary-500 dark:text-primary-400"
              >
                {lang === 'es'
          ...
          /* Backup Code Form */
          <form
            onSubmit={handleSubmitBackup(handleUseBackupCode)}
            className="space-y-4"
          >
            <div className="mb-4 text-center">
              <div className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-yellow-100 dark:bg-yellow-900">
