...
export class JobFirestoreAdapter implements DataAdapter<Job> {
  ...
  async fetch(params: FetchParams): Promise<FetchResult<Job>> {
    ...
    const constraints = this.buildFetchConstraints(
      params,
      fetchSize,
      isMatchSort
    );
  ...
  private mapDoc(id: string, data: Record<string, unknown>): Job {
    ...
    return {
      ...
      employmentType:
        (data['employmentType'] as Job['employmentType']) ?? 'full-time',
      ...
      postedAt:
        (data['postedAt'] as { toDate: () => Date } | null)?.toDate() ??
        new Date(),
  ...
  private buildFetchConstraints(
    ...
    const constraints = this.buildBaseConstraints(
      params.filters as JobFilters | undefined
    );
    ...
    if (!skipSort) {
      ...
      constraints.push(
        orderBy(
          sortField && sortField !== MATCH_SORT_FIELD ? sortField : 'postedAt',
          sortDir
        )
      );
    }
  ...
  private passesEmploymentTypeFilter(job: Job, filters: JobFilters): boolean {
    if (!filters.employmentType || filters.employmentType.length <= 1)
      return true;
  ...
  private passesSalaryFilter(job: Job, filters: JobFilters): boolean {
    ...
    const hasSalaryMax =
      filters.salaryMax !== undefined && filters.salaryMax < 200000;
    ...
    if (
      ...
      max < filters.salaryMin
    )
      return false;
    if (
      hasSalaryMax &&
      filters.salaryMax !== undefined &&
      min > filters.salaryMax
    )
      return false;
    return true;
  }
