...
export class SignUpPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.graduationYearSelect = page.locator(
      '[data-testid="signup-graduation-year"]'
    );
    this.degreeSelect = page.locator('[data-testid="signup-degree"]');
    this.specializationSelect = page.locator(
      '[data-testid="signup-specialization"]'
    );
    ...
    this.showConfirmPasswordToggle = page.locator(
    ...
    this.passwordMismatchMessage = page.locator(
  ...
  async fillAcademicInfo(userData: TestUserData) {
    if (userData.graduationYear) {
      await this.graduationYearSelect.selectOption(
  ...
  async submitForm() {
    ...
    await Promise.race([
      this.page
        .waitForURL(/\/(onboarding|verify-email|dashboard)/, { timeout: 30000 })
        .catch(() => {}),
      this.errorMessage
  ...
  async getPasswordStrength(): Promise<'weak' | 'medium' | 'strong' | null> {
    if (await this.passwordStrengthIndicator.isVisible()) {
      const strengthClass =
        await this.passwordStrengthIndicator.getAttribute('class');
