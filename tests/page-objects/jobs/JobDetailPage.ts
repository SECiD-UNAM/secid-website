...
export class JobDetailPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.applicationDeadline = page.locator(
    ...
    this.availabilityDateInput = page.locator(
      '[data-testid="availability-date"]'
    );
    this.additionalInfoTextarea = page.locator(
    ...
    this.cancelApplicationButton = page.locator(
      '[data-testid="cancel-application"]'
    );

    // Quick apply
    this.quickApplyModal = page.locator('[data-testid="quick-apply-modal"]');
    this.quickApplyResumeSelect = page.locator(
      '[data-testid="quick-apply-resume"]'
    );
    this.quickApplyCoverLetterSelect = page.locator(
      '[data-testid="quick-apply-cover-letter"]'
    );
    this.quickApplySubmitButton = page.locator(
      '[data-testid="quick-apply-submit"]'
    );

    // Application tracking
    this.applicationStatusBadge = page.locator(
      '[data-testid="application-status"]'
    );
    this.applicationTrackingSection = page.locator(
      '[data-testid="application-tracking"]'
    );
    ...
    this.withdrawApplicationButton = page.locator(
    ...
    this.averageResponseTime = page.locator(
    ...
    // Mobile elements
    this.mobileApplyButton = page.locator(
      '[data-testid="mobile-apply-button"]'
    );
    this.mobileJobActions = page.locator('[data-testid="mobile-job-actions"]');
    this.mobileCompanyInfo = page.locator(
      '[data-testid="mobile-company-info"]'
    );
  }
   ...
   */
  async shareJob(
    platform: 'linkedin' | 'twitter' | 'facebook' | 'email' | 'copy-link'
  ) {
  ...
  async getApplicationProgress(): Promise<
    Array<{ step: string; status: string; date?: string }>
  > {
    ...
    for (const stepElement of stepElements) {
      ...
      const stepStatus = await stepElement
        .locator('.step-status')
        .textContent();
  ...
  async reportJob(reason: string) {
    ...
    const submitReportButton = this.page.locator(
  ...
  private async isJobBookmarked(): Promise<boolean> {
    return (
      (await this.bookmarkButton.getAttribute('aria-pressed')) === 'true' ||
  ...
  async validateJobDetailAccessibility() {
    ...
    const applyButtonAriaLabel =
      await this.applyButton.getAttribute('aria-label');
    expect(applyButtonAriaLabel).toBeTruthy();

    // Check bookmark button accessibility
    const bookmarkAriaLabel =
      await this.bookmarkButton.getAttribute('aria-label');
