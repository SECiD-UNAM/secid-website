...
class PaymentFlow {
  ...
  async selectSubscriptionPlan() {
    ...
    await expect(
      this.page.locator('[data-testid="pricing-plans"]')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('[data-testid="plan-details-modal"]')
    ).toBeVisible();
  ...
  async fillBillingInformation() {
    ...
    await this.page.fill(
      ...
      paymentTestData.billingInfo.firstName
    );
    await this.page.fill(
      'input[name="lastName"]',
      paymentTestData.billingInfo.lastName
    );
    await this.page.fill(
      'input[name="email"]',
      paymentTestData.billingInfo.email
    );
    await this.page.fill(
      'input[name="company"]',
      paymentTestData.billingInfo.company
    );
    await this.page.fill(
      'input[name="taxId"]',
      paymentTestData.billingInfo.taxId
    );

    // Fill address
    await this.page.fill(
      'input[name="addressLine1"]',
      paymentTestData.billingInfo.address.line1
    );
    await this.page.fill(
      'input[name="addressLine2"]',
      paymentTestData.billingInfo.address.line2
    );
    await this.page.fill(
      'input[name="city"]',
      paymentTestData.billingInfo.address.city
    );
    await this.page.selectOption(
      'select[name="state"]',
      paymentTestData.billingInfo.address.state
    );
    await this.page.fill(
      'input[name="postalCode"]',
      paymentTestData.billingInfo.address.postalCode
    );
    await this.page.selectOption(
      'select[name="country"]',
      paymentTestData.billingInfo.address.country
    );
  }
  ...
  async fillPaymentDetails() {
    ...
    const cardElement = this.page.frameLocator(
      '[data-testid="stripe-card-element"] iframe'
    );
    await cardElement
      .locator('input[name="cardnumber"]')
      .fill(paymentTestData.creditCard.number);
    await cardElement
      .locator('input[name="exp-date"]')
      .fill(paymentTestData.creditCard.expiry);
    await cardElement
      .locator('input[name="cvc"]')
      .fill(paymentTestData.creditCard.cvc);
    await cardElement
      .locator('input[name="postal"]')
      .fill(paymentTestData.creditCard.zip);
  }

  async reviewOrderSummary() {
    // Check order summary
    await expect(
      this.page.locator('[data-testid="order-summary"]')
    ).toBeVisible();
    await expect(this.page.locator('[data-testid="plan-name"]')).toContainText(
      'SECiD Premium'
    );
    await expect(this.page.locator('[data-testid="plan-price"]')).toContainText(
      '$299.00 MXN'
    );
    ...
    ).toContainText('$346.84 MXN');

    // Check billing cycle
    await expect(
      this.page.locator('[data-testid="billing-cycle"]')
    ).toContainText('Facturación mensual');
  }
  ...
  async completePayment() {
    ...
    await expect(
      this.page.locator('[data-testid="payment-processing"]')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('text=¡Pago procesado exitosamente!')
    ).toBeVisible();
    await expect(
      this.page.locator('text=Tu suscripción está activa')
    ).toBeVisible();
  }
  ...
  async verifySubscriptionActivation() {
    ...
    await expect(
      this.page.locator('[data-testid="subscription-plan"]')
    ).toContainText('SECiD Premium');
    await expect(
  ...
  async purchaseEventTicket() {
    ...
    await expect(
      this.page.locator('[data-testid="ticket-price"]')
    ).toContainText('$750.00 MXN');
    ...
    const fileInput = this.page.locator(
      'input[type="file"][name="studentVerification"]'
    );
  ...
  async purchaseJobPostingPackage() {
    ...
    await expect(
      this.page.locator('[data-testid="package-features"]')
    ).toBeVisible();
  ...
  async handlePaymentFailure() {
    ...
    await expect(
      this.page.locator('text=Tu tarjeta fue rechazada')
    ).toBeVisible();

    // Should allow retry
    await expect(
      this.page.locator('button[data-testid="retry-payment"]')
    ).toBeVisible();
  }
  ...
  async manageSubscription() {
    ...
    await expect(
      this.page.locator('[data-testid="proration-details"]')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('text=Plan actualizado exitosamente')
    ).toBeVisible();
  }
  ...
  async cancelSubscription() {
    ...
    await expect(
      this.page.locator('[data-testid="cancellation-modal"]')
    ).toBeVisible();

    // Show cancellation survey
    await this.page.selectOption(
      'select[name="cancellationReason"]',
      'too-expensive'
    );
    await this.page.fill(
      'textarea[name="feedback"]',
      'The service is great but too expensive for me right now.'
    );

    // Offer retention discount
    await expect(
      this.page.locator('[data-testid="retention-offer"]')
    ).toBeVisible();
    await expect(
      this.page.locator('text=50% de descuento por 3 meses')
    ).toBeVisible();
  ...
  async viewBillingHistory() {
    ...
    await expect(
      this.page.locator('[data-testid="billing-history"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="invoice-item"]')
    ...
    await expect(
      this.page.locator('[data-testid="invoice-modal"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="invoice-total"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="tax-breakdown"]')
    ).toBeVisible();
  }
  ...
  async updatePaymentMethod() {
    ...
    await expect(
      this.page.locator('[data-testid="payment-method-form"]')
    ).toBeVisible();

    // Add new card
    const newCardElement = this.page.frameLocator(
      '[data-testid="new-card-element"] iframe'
    );
    await newCardElement
      .locator('input[name="cardnumber"]')
      .fill('4000000000000077'); // New test card
    ...
    // Should show success message
    await expect(
      this.page.locator('text=Método de pago actualizado')
    ).toBeVisible();

    // Should show new card as default
    await expect(
      this.page.locator('[data-testid="default-payment-method"]')
    ).toContainText('****0077');
  }
}
...
test.describe('Payment and Subscription Flow', () => {
  ...
  test('event ticket purchase flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="ticket-type"]')).toContainText(
      'Estudiante'
    );
    await expect(page.locator('[data-testid="ticket-price"]')).toContainText(
      '$750.00 MXN'
    );
    ...
    await expect(
      page.locator('text=¡Ticket comprado exitosamente!')
    ).toBeVisible();
  ...
  test('job posting package purchase', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="job-title"]')).toContainText(
      'Senior Data Scientist'
    );
    await expect(page.locator('[data-testid="package-name"]')).toContainText(
      'Premium Job Posting'
    );
    await expect(page.locator('[data-testid="package-price"]')).toContainText(
      '$1,999.00 MXN'
    );
    ...
    await expect(
      page.locator('text=¡Trabajo publicado exitosamente!')
    ).toBeVisible();
    await expect(
      page.locator('text=Tu trabajo estará destacado por 30 días')
    ).toBeVisible();
  });
  ...
  test('payment failure handling and retry', async ({ page }) => {
    ...
    const cardElement = page.frameLocator(
      '[data-testid="stripe-card-element"] iframe'
    );
    await cardElement
      .locator('input[name="cardnumber"]')
  ...
  test('Mexican tax calculation and invoicing', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tax-info"]')).toContainText(
      'IVA (16%)'
    );
  });
  ...
  test('multiple currency support', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="premium-price"]')).toContainText(
      '$15.99 USD'
    );
  ...
  test('promotional codes and discounts', async ({ page }) => {
    ...
    // Should show error
    await expect(
      page.locator('text=Código promocional inválido')
    ).toBeVisible();
  });
});
...
test.describe('Payment Accessibility', () => {
  test('pricing page accessibility', async ({ page }) => {
    ...
    // Test ARIA labels
    await expect(
      page.locator('button[data-testid="select-premium-plan"]')
    ).toHaveAttribute('aria-label', 'Select SECiD Premium plan');

    // Test screen reader support
    await expect(page.locator('[data-testid="pricing-card"]')).toHaveAttribute(
      'role',
      'article'
    );
  });
