    ...
    test('should manage team members for corporate plans', async () => {
      ...
      if (teamMembers.length >= 0) {
        ...
        if (inviteResult.success) {
          ...
          const removeResult =
            await subscriptionPage.removeTeamMember('newmember@test.com');
  ...
  test.describe('Billing Management Tests', () => {
    ...
    test('should list invoices correctly', async () => {
      ...
      for (const invoice of invoices) {
        ...
        expect(['paid', 'pending', 'overdue', 'cancelled']).toContain(
          invoice.status
        );
      }
    });
    ...
    test('should download invoice successfully', async () => {
      ...
      if (invoices.length > 0) {
        const downloadResult = await billingPage.downloadInvoice(
          invoices[0].number
        );
    ...
    test('should display payment history', async () => {
      ...
      for (const payment of payments) {
        ...
        expect(['completed', 'failed', 'pending', 'refunded']).toContain(
          payment.status
        );
      }
    });
    ...
    test('should manage team member lifecycle', async () => {
      ...
      if (inviteResult.success) {
        ...
        if (newMember && newMember.status === 'active') {
          // Remove member
          const removeResult =
            await subscriptionPage.removeTeamMember('team@test.com');
          expect(removeResult.success).toBe(true);
        }
      }
    });
  ...
  test.describe('Promotional and Discount Scenarios', () => {
    test('should apply student discount correctly', async () => {
      ...
      if (promoResult.success) {
        ...
        const expectedDiscount = orderSummary.basePrice * 0.3;
        expect(
          Math.abs(orderSummary.discount! - expectedDiscount)
        ).toBeLessThan(0.01);
      }
    });
