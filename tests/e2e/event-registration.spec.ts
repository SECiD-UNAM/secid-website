...
const eventTestData = {
  eventDetails: {
    ...
    description:
      'Comprehensive workshop on modern ML techniques and applications',
...
class EventRegistrationFlow {
  ...
  async browseEvents() {
    ...
    await expect(
      this.page.locator('h1:has-text("Próximos Eventos")')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="event-card"]')
    ...
    await this.page.selectOption(
      'select[data-testid="event-location-filter"]',
      'virtual'
    );
  ...
  async viewEventDetails(eventIndex = 0) {
    ...
    await expect(
      this.page.locator('[data-testid="event-title"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="event-description"]')
    ).toBeVisible();
    await expect(this.page.locator('[data-testid="event-date"]')).toBeVisible();
    await expect(
      this.page.locator('[data-testid="event-location"]')
    ).toBeVisible();
  }
  ...
  async registerForFreeEvent() {
    ...
    await expect(
      this.page.locator('h2:has-text("Registro para el Evento")')
    ).toBeVisible();
    ...
    await this.page.fill(
      ...
      eventTestData.registrationInfo.dietaryRestrictions
    );
    await this.page.fill(
      'textarea[name="accessibilityNeeds"]',
      eventTestData.registrationInfo.accessibilityNeeds
    );
    await this.page.fill(
      'input[name="emergencyContact"]',
      eventTestData.registrationInfo.emergencyContact
    );
    await this.page.fill(
      'textarea[name="additionalInfo"]',
      eventTestData.registrationInfo.additionalInfo
    );

    // Event-specific fields
    await this.page.selectOption(
      'select[name="tshirtSize"]',
      eventTestData.registrationInfo.tshirtSize
    );
    ...
    await expect(
      this.page.locator('text=Te hemos enviado un email de confirmación')
    ).toBeVisible();
  }
  ...
  async registerForPaidEvent() {
    ...
    await expect(
      this.page.locator('[data-testid="pricing-options"]')
    ).toBeVisible();
    ...
    const fileInput = this.page.locator(
      'input[type="file"][name="studentIdDocument"]'
    );
  ...
  async manageEventRegistration() {
    ...
    await expect(
      this.page.locator('[data-testid="registered-event"]')
    ).toBeVisible();

    const registeredEvent = this.page
      .locator('[data-testid="registered-event"]')
      .first();
    ...
    await expect(
      this.page.locator('[data-testid="registration-details-modal"]')
    ).toBeVisible();
  ...
  async cancelEventRegistration() {
    ...
    await registeredEvent
      .locator('[data-testid="cancel-registration"]')
      .click();

    // Should show confirmation dialog
    await expect(
      this.page.locator('[data-testid="cancel-confirmation-dialog"]')
    ).toBeVisible();
    await expect(
      this.page.locator(
        'text=¿Estás seguro de que quieres cancelar tu registro?'
      )
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('text=Registro cancelado exitosamente')
    ).toBeVisible();
  ...
  async addEventToCalendar() {
    ...
    await expect(
      this.page.locator('[data-testid="calendar-options"]')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('text=Agregando a Google Calendar')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('text=Descargando archivo de calendario')
    ).toBeVisible();
  }
  ...
  async shareEvent() {
    ...
    await expect(
      this.page.locator('[data-testid="share-modal"]')
    ).toBeVisible();

    // Test copy link
    await this.page.click('[data-testid="copy-event-link"]');
    await expect(
      this.page.locator('text=Enlace copiado al portapapeles')
    ).toBeVisible();
  ...
  async checkInToEvent() {
    ...
    await this.page.goto(
      '/es/events/check-in?eventId=test-event-id&code=abc123'
    );

    // Should show check-in interface
    await expect(
      this.page.locator('h2:has-text("Check-in para el Evento")')
    ).toBeVisible();

    // Verify registration
    await expect(this.page.locator('text=Ana García')).toBeVisible();
    await expect(
      this.page.locator('text=Machine Learning Workshop 2024')
    ).toBeVisible();
  ...
  async viewEventAttendees() {
    ...
    await expect(
      this.page.locator('[data-testid="attendees-modal"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="attendee-item"]')
    ).toHaveCountGreaterThan(0);

    // Test networking features
    const firstAttendee = this.page
    ...
    await expect(
      this.page.locator('text=Solicitud de conexión enviada')
    ).toBeVisible();

    // Message attendee (if connected)
    await firstAttendee.locator('[data-testid="message-attendee"]').click();
    await expect(
      this.page.locator('[data-testid="message-modal"]')
    ).toBeVisible();
  }
  ...
  async provideEventFeedback() {
    ...
    await expect(
      this.page.locator('[data-testid="feedback-modal"]')
    ).toBeVisible();
    ...
    await this.page.fill(
      ...
      'Excellent workshop! Learned a lot about ML techniques.'
    );
    ...
    await expect(
      this.page.locator('text=¡Gracias por tu feedback!')
    ).toBeVisible();
  }
}
...
test.describe('Event Registration and Management Flow', () => {
  ...
  test('event browsing and filtering', async ({ page }) => {
    ...
    for (const filter of filters) {
      await page.selectOption(
        `select[data-testid="event-${filter.type}-filter"]`,
        filter.value
      );
      ...
      await expect(
        page.locator(`[data-testid="active-filter-${filter.type}"]`)
      ).toContainText(filter.value);
    }

    // Test clearing filters
    await page.click('button[data-testid="clear-all-filters"]');
    await expect(page.locator('[data-testid^="active-filter-"]')).toHaveCount(
      0
    );
  });
  ...
  test('paid event registration flow', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Documentación estudiantil en revisión')
    ).toBeVisible();

    // Payment would be handled by Stripe component
    await expect(
      page.locator('[data-testid="stripe-payment-element"]')
    ).toBeVisible();
  });
  ...
  test('event capacity and waiting list', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="capacity-indicator"]')
    ).toContainText('50/50');

    // Should offer waiting list option
    await expect(
      page.locator('button[data-testid="join-waiting-list"]')
    ).toBeVisible();
    ...
    await expect(
      page.locator('text=Te hemos agregado a la lista de espera')
    ).toBeVisible();
  });
  ...
  test('event check-in process', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Ya has hecho check-in para este evento')
    ).toBeVisible();
  });
  ...
  test('event networking features', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="attendee-item"]')
    ).toHaveCountGreaterThan(0);
    ...
    // Should show filtered results
    await expect(
      page.locator('[data-testid="filtered-attendees"]')
    ).toBeVisible();
  });
  ...
  test('event feedback and rating', async ({ page }) => {
    ...
    await expect(
      pastEvent.locator('[data-testid="feedback-provided"]')
    ).toBeVisible();
    await expect(
      pastEvent.locator('[data-testid="event-rating"]')
    ).toContainText('5');
  });
  ...
  test('event cancellation scenarios', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Cancelación gratuita hasta 24 horas antes')
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="event-cancelled-notice"]')
    ).toBeVisible();
    await expect(
      page.locator('text=Este evento ha sido cancelado')
    ).toBeVisible();
  });
...
test.describe('Mobile Event Registration', () => {
  ...
  test('mobile event browsing and registration', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="mobile-event-details"]')
    ).toBeVisible();
  ...
  test('mobile event check-in', async ({ page }) => {
    ...
    const checkinButton = page.locator(
      'button[data-testid="complete-checkin"]'
    );
...
test.describe('Event Registration Accessibility', () => {
  test('events page accessibility', async ({ page }) => {
    ...
    await expect(
      page.locator('input[data-testid="event-search"]')
    ).toBeFocused();

    // Test ARIA labels
    await expect(
      page.locator('button[data-testid="apply-event-filters"]')
    ).toHaveAttribute('aria-label', 'Apply event filters');
