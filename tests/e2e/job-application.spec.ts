...
const testJobApplication = {
  coverLetter:
    'I am very interested in this Data Scientist position. My experience with machine learning and Python aligns perfectly with your requirements...',
  resume: 'resume.pdf',
  portfolio: 'https://github.com/testuser/portfolio',
  availableStartDate: '2024-03-01',
  salaryExpectation: '80000',
  additionalInfo:
    'I am available for immediate interviews and can relocate if needed.',
};
...
class JobApplicationFlow {
  ...
  async searchJobs(searchTerm: string) {
    const searchInput = this.page.locator(
      'input[data-testid="job-search-input"]'
    );
  ...
  async applyAdvancedFilters() {
    ...
    // Apply salary range
    await this.page.fill(
      'input[data-testid="salary-min"]',
      advancedFilters.salaryMin
    );
    await this.page.fill(
      'input[data-testid="salary-max"]',
      advancedFilters.salaryMax
    );

    // Select experience level
    await this.page.selectOption(
      'select[data-testid="experience-level"]',
  ...
  async startJobApplication() {
    ...
    await expect(
      this.page.locator('h2:has-text("Aplicar para este trabajo")')
    ).toBeVisible();
  }
  ...
  async fillApplicationForm() {
    ...
    await this.page.fill(
      ...
      testJobApplication.coverLetter
    );
    ...
    await this.page.fill(
      ...
      testJobApplication.portfolio
    );

    // Set availability date
    await this.page.fill(
      'input[name="availableStartDate"]',
      testJobApplication.availableStartDate
    );

    // Set salary expectation
    await this.page.fill(
      'input[name="salaryExpectation"]',
      testJobApplication.salaryExpectation
    );

    // Add additional information
    await this.page.fill(
      'textarea[name="additionalInfo"]',
      testJobApplication.additionalInfo
    );
  }
  ...
  async submitApplication() {
    ...
    await expect(
      this.page.locator('text=¡Aplicación enviada exitosamente!')
    ).toBeVisible();
  ...
  async viewApplicationStatus() {
    ...
    await expect(
      this.page.locator('[data-testid="application-item"]')
    ).toBeVisible();
  ...
  async saveJobForLater(jobIndex = 0) {
    const jobCards = this.page.locator('[data-testid="job-card"]');
    const saveButton = jobCards
      .nth(jobIndex)
      .locator('button[data-testid="save-job"]');
  ...
  async viewSavedJobs() {
    ...
    await expect(
      this.page.locator('[data-testid="saved-job-item"]')
    ).toBeVisible();
  }
}
...
test.describe('Job Application and Search Flow', () => {
  ...
  test('job search functionality', async ({ page }) => {
    ...
    ).toContainText('3 trabajos encontrados');
  ...
  test('job application form validation', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Please upload a PDF or DOC file')
    ).toBeVisible();
    ...
    await page.click('button[data-testid="submit-application"]');
    await expect(
      page.locator('text=Cover letter must be at least 50 characters')
    ).toBeVisible();
  });
  ...
  test('job alerts and notifications', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="job-alert-item"]')).toContainText(
      'Data Science Jobs'
    );
  });
  ...
  test('application tracking and status updates', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="application-item"]')).toHaveCount(
      1
    );
    ...
    await expect(
      page.locator('h3:has-text("Detalles de la aplicación")')
    ).toBeVisible();

    // Should show application timeline
    await expect(
      page.locator('[data-testid="application-timeline"]')
    ).toBeVisible();
  ...
  test('company profile and jobs', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="company-location"]')
    ).toBeVisible();

    // Should show other jobs from this company
    await expect(
      page.locator('h3:has-text("Otros trabajos en esta empresa")')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="company-job-item"]')
    ).toBeVisible();
  ...
  test('job recommendations', async ({ page }) => {
    ...
    await expect(
      page.locator('h3:has-text("Trabajos recomendados")')
    ).toBeVisible();
...
test.describe('Mobile Job Application', () => {
  ...
  test('mobile job filters', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="filters-modal"]')
    ).not.toBeVisible();
