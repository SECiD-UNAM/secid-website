...
class LearningPlatformFlow {
  ...
  async enrollInCourse(courseTitle: string) {
    ...
    await this.page.click(
      `[data-testid="course-card"]:has-text("${courseTitle}")`
    );

    // View course details
    await expect(this.page.locator('h1')).toContainText(courseTitle);
    await expect(
      this.page.locator('[data-testid="course-modules"]')
    ).toBeVisible();
    ...
    // Verify enrollment
    await expect(
      this.page.locator('[data-testid="enrollment-success"]')
    ).toBeVisible();
  }
  ...
  async startLearning() {
    ...
    // Select a course
    await this.page.click(
      '[data-testid="course-card"]:first-child [data-testid="continue-learning"]'
    );

    // Start first module
    await this.page.click(
      '[data-testid="module-1"] [data-testid="start-module"]'
    );
  }
  ...
  async completeLesson() {
    ...
    if (
      await this.page.locator('[data-testid="lesson-activity"]').isVisible()
    ) {
    ...
    await expect(
      this.page.locator('[data-testid="progress-bar"]')
  ...
  async takeQuiz(quiz: typeof learningTestData.quiz) {
    ...
    for (let i = 0; i < quiz.questions.length; i++) {
      ...
      await this.page.click(
        `[data-testid="question-${i}"] [data-testid="option-${question.correct}"]`
      );
    ...
    await expect(
      this.page.locator('[data-testid="quiz-feedback"]')
    ).toBeVisible();
  }
  ...
  async exploreLearningPaths() {
    ...
    await expect(
      this.page.locator('[data-testid="learning-path-card"]')
    ).toHaveCount(5);
  ...
  async enrollInLearningPath(pathTitle: string) {
    await this.page.click(
      `[data-testid="learning-path-card"]:has-text("${pathTitle}")`
    );

    // View path details
    await expect(
      this.page.locator('[data-testid="path-roadmap"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="path-courses"]')
    ).toBeVisible();
    ...
    // Verify enrollment
    await expect(
      this.page.locator('[data-testid="path-progress"]')
    ).toBeVisible();
  }
  ...
  async viewCertifications() {
    ...
    await expect(
      this.page.locator('[data-testid="certification-card"]')
    ).toHaveCount(3);
  }

  async startCertification(certName: string) {
    await this.page.click(
      `[data-testid="certification-card"]:has-text("${certName}")`
    );

    // View requirements
    await expect(
      this.page.locator('[data-testid="cert-requirements"]')
    ).toBeVisible();
    ...
    if (
      await this.page.locator('[data-testid="eligible-status"]').isVisible()
    ) {
  ...
  async takeAssessment() {
    ...
    for (let i = 0; i < 20; i++) {
      await this.page.click(
        `[data-testid="assessment-q-${i}"] [data-testid="option-0"]`
      );
  ...
  async participateInDiscussion() {
    ...
    await this.page.fill(
      ...
      'Question about neural network initialization'
    );
    await this.page.fill(
      '[data-testid="discussion-body"]',
      'Can someone explain the benefits of Xavier initialization?'
    );
    ...
    await this.page.fill(
      ...
      'Great question! Xavier initialization helps with...'
    );
  ...
  async submitAssignment() {
    ...
    await this.page.setInputFiles(
      '[data-testid="assignment-upload"]',
      'tests/fixtures/assignment-solution.ipynb'
    );

    // Add description
    await this.page.fill(
      '[data-testid="assignment-description"]',
      'Implemented the CNN model as requested with 95% accuracy'
    );
  ...
  async downloadCertificate() {
    ...
    const certificateCard = this.page.locator(
      '[data-testid="completed-certificate"]:first-child'
    );
  ...
  async viewLearningAnalytics() {
    ...
    await expect(
      this.page.locator('[data-testid="learning-streak"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="courses-completed"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="certificates-earned"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="total-learning-hours"]')
    ).toBeVisible();
  ...
  async setLearningGoals() {
    ...
    // Verify goals saved
    await expect(
      this.page.locator('[data-testid="goals-saved"]')
    ).toBeVisible();
  }
}
...
test.describe('Learning Platform', () => {
  ...
  test('Learning path enrollment', async () => {
    ...
    await learningFlow.enrollInLearningPath(
      learningTestData.learningPath.title
    );
  });
  ...
  test('Mobile learning experience', async () => {
    ...
    await expect(
      page.locator('[data-testid="mobile-course-nav"]')
    ).toBeVisible();
