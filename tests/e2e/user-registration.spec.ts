...
class RegistrationFlow {
  ...
  async completeEmailVerification() {
    ...
    await expect(
      this.page.locator('text=Verificación de email enviada')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('text=Email verificado exitosamente')
    ).toBeVisible();
  }
  ...
  async viewConnectionSuggestions() {
    ...
    const connectionButtons = this.page.locator(
...
test.describe('User Registration and Onboarding Flow', () => {
  ...
  test('email verification resend', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Verificación de email enviada')
    ).toBeVisible();
    ...
    await expect(
      page.locator('text=Email de verificación reenviado')
    ).toBeVisible();

    // Should disable button temporarily
    await expect(
      page.locator('button[data-testid="resend-verification"]')
    ).toBeDisabled();
  });
  ...
  test('profile setup form validation', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Graduation year is required')
    ).toBeVisible();
...
test.describe('Cross-browser Registration Tests', () => {
  ['chromium', 'firefox', 'webkit'].forEach((browserName) => {
    test(`registration works in ${browserName}`, async ({
      page,
      browserName: currentBrowser,
    }) => {
      test.skip(
        currentBrowser !== browserName,
        `Test is for ${browserName} only`
      );
      ...
      await expect(
        page.locator(
          '[data-testid="loading-spinner"], text=Verificación de email enviada'
        )
      ).toBeVisible();
    });
  });
...
test.describe('Mobile Registration Tests', () => {
  ...
  test('mobile registration flow', async ({ page }) => {
    ...
    await expect(
      page.locator('button[data-testid="mobile-menu-toggle"]')
    ).toBeVisible();
