...
test.describe('Social Authentication', () => {
  ...
  test.describe('Google Authentication', () => {
    ...
    test('should handle Google OAuth errors', async ({ page }) => {
      ...
      await expect(errorMessage).toContainText(
        /acceso denegado|error|cancelado/i
      );
    });
  });
  ...
  test.describe('GitHub Authentication', () => {
    ...
    test('should verify GitHub email during OAuth', async ({ page }) => {
      ...
      await page.goto(
        '/oauth-callback?provider=github&code=mock-code&email_verified=false'
      );
      ...
      await expect(message).toContainText(
        /verificar.*correo|email.*verification/i
      );
    });
  });
  ...
  test.describe('OAuth Account Linking', () => {
    test('should link OAuth account to existing user', async ({ page }) => {
      ...
      const linkGoogleButton = page.locator(
      ...
      if (await linkGoogleButton.isVisible()) {
        ...
        await expect(
          page.locator('[data-testid="google-linked"]')
        ).toBeVisible();
      }
    });
    ...
    test('should handle OAuth account already linked', async ({ page }) => {
      ...
      await page.goto(
        '/oauth-callback?provider=google&error=account_already_linked'
      );

      // Should show appropriate message
      const message = page.locator('[data-testid="oauth-message"]');
      await expect(message).toContainText(
        /cuenta.*ya.*vinculada|already.*linked/i
      );
    });
    ...
    test('should merge accounts when email matches', async ({ page }) => {
      ...
      await page.goto(
        '/oauth-callback?provider=google&email=test@secid.mx&merge_account=true'
      );
  ...
  test.describe('OAuth Security', () => {
    ...
    test('should expire OAuth state after timeout', async ({ page }) => {
      ...
      await page.goto(
        '/oauth-callback?provider=google&code=mock-code&state=expired-state'
      );
    ...
    test('should prevent OAuth replay attacks', async ({ page }) => {
      const callbackUrl =
        '/oauth-callback?provider=google&code=used-code&state=valid-state';
      ...
      await expect(error).toContainText(
        /código.*usado|already.*used|inválido/i
      );
    });
  });
  ...
  test.describe('OAuth User Experience', () => {
    test('should show loading state during OAuth', async ({ page }) => {
      ...
      await page.goto(
        '/oauth-callback?provider=google&code=mock-code&state=valid'
      );
      ...
      await expect(loading).toContainText(
        /procesando|processing|autenticando/i
      );
    });

    test('should remember OAuth provider preference', async ({
      page,
      context,
    }) => {
      // Login with Google
      ...
      const isPromoted = await googleButton.evaluate((el) => {
        return (
          el.classList.contains('promoted') ||
    ...
    test('should handle popup blockers gracefully', async ({ page }) => {
      ...
      await expect(message).toContainText(
        /bloqueador.*popups|popup.*blocker|ventana/i
      );
    });
  });
