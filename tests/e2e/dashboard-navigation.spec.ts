...
class DashboardNavigationFlow {
  ...
  async verifyDashboardLayout() {
    ...
    await expect(
      this.page.locator('[data-testid="dashboard-sidebar"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="dashboard-main-content"]')
    ).toBeVisible();
    ...
    await expect(
      this.page.locator('[data-testid="user-welcome"]')
  ...
  async testQuickActions() {
    ...
    const quickActionButtons = this.page.locator(
    ...
    for (let i = 0; i < count; i++) {
      ...
      const modalVisible = await this.page
        .locator('[data-testid="modal"], [data-testid="drawer"]')
        .isVisible();
      const urlChanged =
        this.page.url() !== 'http://localhost:4321/es/dashboard';
  ...
  async testNotificationsCenter() {
    ...
    await expect(
      this.page.locator('[data-testid="notifications-dropdown"]')
    ).toBeVisible();
  ...
  async testDashboardStats() {
    ...
    // Check stats cards
    await expect(
      this.page.locator('[data-testid="stats-connections"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="stats-applications"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="stats-messages"]')
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="stats-events"]')
    ).toBeVisible();

    // Verify stats have numeric values
    const connectionsCount = await this.page
      .locator('[data-testid="connections-count"]')
      .textContent();
  ...
  async testRecentActivity() {
    ...
    await expect(
      this.page.locator('h3:has-text("Actividad Reciente")')
    ).toBeVisible();
  ...
  async testUpcomingEvents() {
    ...
    await expect(
      this.page.locator('[data-testid="upcoming-events"]')
    ).toBeVisible();
  ...
  async testJobMatches() {
    ...
    await expect(
      this.page.locator('[data-testid="job-matches"]')
    ).toBeVisible();
  ...
  async testBreadcrumbNavigation() {
    ...
    // Check breadcrumbs
    await expect(
      this.page.locator('[data-testid="breadcrumbs"]')
    ).toBeVisible();
  ...
  async testMobileNavigation() {
    ...
    for (const menuItem of dashboardTestData.menuItems.slice(0, 3)) {
      await this.page.click(
        `[data-testid="mobile-menu"] a[href="${menuItem.url}"]`
      );
      await expect(this.page).toHaveURL(menuItem.url);
  ...
  async testSearchFunctionality() {
    ...
    await expect(
      this.page.locator('[data-testid="search-suggestions"]')
    ).toBeVisible();
...
test.describe('Dashboard Navigation and Functionality', () => {
  ...
  test('sidebar navigation and states', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="dashboard-sidebar"]')).toHaveClass(
      /collapsed/
    );
    ...
    await expect(page.locator('a[href="/es/dashboard/profile"]')).toHaveClass(
      /active/
    );
    ...
    for (const menuItem of dashboardTestData.menuItems) {
      ...
      await expect(
        menuLink.locator(`[data-icon="${menuItem.icon}"]`)
      ).toBeVisible();
    }
  });
  ...
  test('notifications system', async ({ page }) => {
    ...
    await page.click(
      '[data-testid="notification-item"] [data-testid="view-details"]'
    );
  ...
  test('dashboard stats and metrics', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="loading-stats"]')
    ).not.toBeVisible();
  });
  ...
  test('dashboard customization', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="dashboard-grid"]')).toHaveClass(
      /custom-layout/
    );
    ...
    await expect(
      page.locator('[data-testid="widget-activity"]')
    ).not.toBeVisible();
...
test.describe('Mobile Dashboard Navigation', () => {
  ...
  test('mobile dashboard layout', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="mobile-quick-actions"]')
    ).toBeVisible();
  });
  ...
  test('mobile notifications', async ({ page }) => {
    ...
    await page.click('[data-testid="notifications-button"]');
    await expect(
      page.locator('[data-testid="mobile-notifications-fullscreen"]')
    ).toBeVisible();
...
test.describe('Tablet Dashboard Navigation', () => {
  ...
  test('tablet dashboard layout', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tablet-stats-grid"]')).toHaveClass(
      /grid-2x2/
    );
  });
});
...
test.describe('Dashboard Accessibility', () => {
  ...
  test('dashboard screen reader support', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="dashboard-main"]')
    ).toHaveAttribute('role', 'main');
    await expect(
      page.locator('[data-testid="dashboard-sidebar"]')
    ).toHaveAttribute('role', 'navigation');
    ...
    await expect(
      page.locator('[data-testid="stats-connections"]')
    ).toHaveAttribute('aria-label', '45 connections');
