...
const communityTestData = {
  forumPost: {
    ...
    content:
      'I wanted to share some insights from deploying ML models at scale. Here are key considerations...',
  ...
  blogPost: {
    ...
    content:
      "After 5 years in software engineering, I decided to make the leap into data science. Here's my story...",
  ...
  mentorshipRequest: {
    ...
    description:
      'Looking for guidance on transitioning from data analyst to ML engineer role',
...
class CommunityFeaturesFlow {
  ...
  async interactWithForumPost() {
    ...
    await expect(this.page.locator('[data-testid="like-count"]')).toHaveText(
      '1'
    );
    ...
    await expect(
      this.page.locator('[data-testid="comment-list"] [data-testid="comment"]')
    ).toHaveCount(1);

    // Follow the author
    await this.page.click('[data-testid="follow-author"]');
    await expect(
      this.page.locator('[data-testid="follow-button"]')
    ).toContainText('Siguiendo');
  }
  ...
  async searchForumPosts(query: string) {
    ...
    const results = await this.page
      .locator('[data-testid="forum-post-title"]')
      .allTextContents();
    expect(
      results.some((title) => title.toLowerCase().includes(query.toLowerCase()))
    ).toBeTruthy();
  }
  ...
  async shareResource(resource: typeof communityTestData.resourceShare) {
    ...
    await expect(
      this.page.locator('[data-testid="success-message"]')
    ).toContainText('Recurso compartido exitosamente');
  }
  ...
  async rateResource() {
    ...
    // Verify review added
    await expect(
      this.page.locator('[data-testid="review-list"] [data-testid="review"]')
    ).toHaveCount(1);
  }
  ...
  async requestMentorship(request: typeof communityTestData.mentorshipRequest) {
    ...
    await this.page.click(
      '[data-testid="mentor-card"]:first-child [data-testid="request-mentorship"]'
    );

    // Fill request details
    await this.page.fill('[data-testid="mentorship-topic"]', request.topic);
    await this.page.fill(
      '[data-testid="mentorship-description"]',
      request.description
    );
    await this.page.selectOption(
      '[data-testid="preferred-schedule"]',
    ...
    await expect(
      this.page.locator('[data-testid="success-message"]')
    ).toContainText('Solicitud de mentoría enviada');
  }
  ...
  async becomeMentor() {
    ...
    await this.page.fill(
      ...
      'Machine Learning, Data Engineering, Career Development'
    );
    await this.page.fill(
      '[data-testid="mentorship-bio"]',
    ...
    await expect(
      this.page.locator('[data-testid="mentor-status"]')
    ).toContainText('Solicitud en revisión');
  }
  ...
  async checkActivityFeed() {
    ...
    await expect(
      this.page.locator('[data-testid="activity-feed"]')
    ).toBeVisible();
    ...
    for (const type of activityTypes) {
      ...
      if (await activity.isVisible()) {
        ...
        await expect(
          activity.locator('[data-testid="activity-actions"]')
        ).toBeVisible();
      }
    }
  }
  ...
  async testNotifications() {
    ...
    await expect(
      this.page.locator('[data-testid="notification-bell"]')
    ).toBeVisible();

    // Open notifications
    await this.page.click('[data-testid="notification-bell"]');
    await expect(
      this.page.locator('[data-testid="notification-dropdown"]')
    ).toBeVisible();

    // Mark as read
    await this.page.click('[data-testid="mark-all-read"]');
    await expect(
      this.page.locator('[data-testid="notification-count"]')
    ).toHaveText('0');
  }
  ...
  async testMessaging() {
    ...
    await this.page.fill(
      ...
      'Hi! I saw your post about ML deployment. Would love to connect!'
    );
    await this.page.click('[data-testid="send-message"]');

    // Verify message sent
    await expect(
      this.page.locator('[data-testid="message-list"] [data-testid="message"]')
    ).toHaveCount(1);
  }
}
...
test.describe('Community Features', () => {
  ...
  test('Mobile community features', async () => {
    ...
    await expect(
      page.locator('[data-testid="mobile-forum-nav"]')
    ).toBeVisible();
    await expect(
