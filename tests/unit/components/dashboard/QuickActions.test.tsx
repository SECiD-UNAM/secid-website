...
describe('QuickActions', () => {
  ...
  describe('Basic Rendering', () => {
    ...
    it('displays action descriptions in Spanish', () => {
      ...
      expect(
        screen.getByText('Explora oportunidades laborales')
      ).toBeInTheDocument();
      expect(
        screen.getByText('Mantén tu perfil actualizado')
      ).toBeInTheDocument();
      expect(
        screen.getByText('Próximos eventos y talleres')
      ).toBeInTheDocument();
    ...
    it('displays action descriptions in English', () => {
      ...
      expect(
        screen.getByText('Keep your profile up to date')
      ).toBeInTheDocument();
      expect(
        screen.getByText('Upcoming events and workshops')
      ).toBeInTheDocument();
  ...
  describe('Navigation Links', () => {
    it('has correct href attributes for Spanish locale', () => {
      ...
      expect(
        screen.getByRole('link', { name: /buscar empleos/i })
      ).toHaveAttribute('href', '/es/dashboard/jobs');
      expect(
        screen.getByRole('link', { name: /actualizar cv/i })
      ).toHaveAttribute('href', '/es/dashboard/profile');
      expect(
        screen.getByRole('link', { name: /ver eventos/i })
      ).toHaveAttribute('href', '/es/dashboard/events');
      expect(
        screen.getByRole('link', { name: /publicar empleo/i })
      ).toHaveAttribute('href', '/es/dashboard/jobs/post');
      expect(screen.getByRole('link', { name: /conectar/i })).toHaveAttribute(
        'href',
        '/es/dashboard/members'
      );
      expect(screen.getByRole('link', { name: /mentoría/i })).toHaveAttribute(
        'href',
        '/es/dashboard/mentorship'
      );
    });
  ...
  describe('Verification State Handling', () => {
    ...
    it('does not disable other actions for unverified users', () => {
      ...
      expect(screen.getByRole('link', { name: /conectar/i })).not.toHaveClass(
        'opacity-50'
      );
    });
  });
  ...
  describe('Dark Mode Support', () => {
    ...
    it('includes dark mode classes for text', () => {
      ...
      expect(screen.getByText('Explora oportunidades laborales')).toHaveClass(
        'dark:text-gray-400'
      );
    });
  ...
  describe('Accessibility', () => {
    ...
    it('has descriptive link text', () => {
      ...
      expect(
        screen.getByRole('link', {
          name: /buscar empleos.*explora oportunidades/i,
        })
      ).toBeInTheDocument();
      expect(
        screen.getByRole('link', { name: /actualizar cv.*mantén tu perfil/i })
      ).toBeInTheDocument();
    });
    ...
    it('provides clear indication of disabled state', () => {
      ...
      expect(
        within(mentorshipLink).getByText('Verificación requerida')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Action Content Structure', () => {
    it('displays action titles and descriptions in proper hierarchy', () => {
      ...
      expect(
        screen.getByRole('heading', { name: 'Buscar Empleos' })
      ).toBeInTheDocument();
      expect(
        screen.getByText('Explora oportunidades laborales')
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByRole('heading', { name: 'Actualizar CV' })
      ).toBeInTheDocument();
      expect(
        screen.getByText('Mantén tu perfil actualizado')
      ).toBeInTheDocument();
  ...
  describe('Color System', () => {
    it('uses consistent color system across actions', () => {
      ...
      iconContainers.forEach((container) => {
        const hasColorClass = Array.from(container.classList).some(
          (className) =>
