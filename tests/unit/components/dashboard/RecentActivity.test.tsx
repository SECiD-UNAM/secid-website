...
describe('RecentActivity', () => {
  ...
  describe('Basic Rendering', () => {
    it('renders activity timeline', async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(
          screen.getByText('Te registraste a un evento')
        ).toBeInTheDocument();
    ...
    it('displays activity descriptions in Spanish', async () => {
      ...
      await vi.waitFor(() => {
        expect(
          screen.getByText('Senior Data Scientist en TechCorp México')
        ).toBeInTheDocument();
        expect(
          screen.getByText('Taller de MLOps - 20 de Febrero')
        ).toBeInTheDocument();
        expect(
          screen.getByText('Agregaste nuevas habilidades y experiencia')
        ).toBeInTheDocument();
        expect(
          screen.getByText('Pregunta sobre transición a la industria')
        ).toBeInTheDocument();
      });
    });
    ...
    it('displays activity descriptions in English', async () => {
      ...
      await vi.waitFor(() => {
        expect(
          screen.getByText('Senior Data Scientist at TechCorp Mexico')
        ).toBeInTheDocument();
        expect(
          screen.getByText('MLOps Workshop - February 20')
        ).toBeInTheDocument();
        expect(
          screen.getByText('Added new skills and experience')
        ).toBeInTheDocument();
        expect(
          screen.getByText('Question about industry transition')
        ).toBeInTheDocument();
      });
    });
    ...
    it('shows verification activity for verified users', async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(
          screen.getByText('Tu cuenta UNAM ha sido verificada')
        ).toBeInTheDocument();
      });
    });
    ...
    it('shows verification activity in English for verified users', async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(
          screen.getByText('Your UNAM account has been verified')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Empty State', () => {
    it('shows empty state when no activities exist', async () => {
      ...
      await vi.waitFor(() => {
        expect(
          screen.getByText('No hay actividad reciente')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Activity Types and Icons', () => {
    ...
    it('handles different activity types correctly', async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(
          screen.getByText('Te registraste a un evento')
        ).toBeInTheDocument(); // event_registration
  ...
  describe('Footer Action Link', () => {
    ...
    it('has proper styling for footer link', async () => {
      ...
      await vi.waitFor(() => {
        const footer = screen
          .getByText('Ver toda la actividad →')
          .closest('div');
  ...
  describe('Dark Mode Support', () => {
    it('includes dark mode classes for main container', async () => {
      ...
      await vi.waitFor(() => {
        const container = screen
          .getByText('Aplicaste a un trabajo')
          .closest('.bg-white');
    ...
    it('includes dark mode classes for text elements', async () => {
      ...
      await vi.waitFor(() => {
        ...
        const activityDescription = screen.getByText(
    ...
    it('includes dark mode classes for timeline elements', async () => {
      ...
      await vi.waitFor(() => {
        const connectors = screen
          .getAllByRole('generic')
          .filter((el) => el.classList.contains('bg-gray-200'));
  ...
  describe('Accessibility', () => {
    ...
    it('has proper ARIA labels for timeline elements', async () => {
      ...
      await vi.waitFor(() => {
        ...
        const hiddenConnectors = connectors.filter(
