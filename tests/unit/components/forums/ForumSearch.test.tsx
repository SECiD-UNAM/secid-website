...
const makeTopic = (
  overrides?: Partial<ForumSearchResult>
): ForumSearchResult => ({
  id: 'topic-1',
  type: 'topic',
...
describe.sequential('ForumSearch', () => {
   ...
   */
  'renders search form with input and submit button', async () => {
    render(<ForumSearch language="en" />);
    // Wait for initial loading to settle
    await waitFor(() => {
      expect(
        screen.getByPlaceholderText('Search forums...')
      ).toBeInTheDocument();
    });
   ...
   */
  'displays topic results after search', async () => {
    ...
    const { container } = render(
      <ForumSearch language="en" initialQuery="Python" />
    );
   ...
   */
  'displays post results after search', async () => {
    mockSearch.mockResolvedValue({
      topics: [],
      posts: [makePost()],
    });
   ...
   */
  'shows result type tabs with counts', async () => {
    mockSearch.mockResolvedValue({
      topics: [makeTopic()],
      posts: [makePost()],
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('button', { name: 'Topics (1)' })
      ).toBeInTheDocument();
      expect(
        screen.getByRole('button', { name: 'Posts (1)' })
      ).toBeInTheDocument();
      expect(
   ...
   */
  'switches to topics tab and shows only topics', async () => {
    mockSearch.mockResolvedValue({
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('button', { name: /Topics \(1\)/i })
      ).toBeInTheDocument();
    });
   ...
   */
  'shows loading indicator while searching', async () => {
    let resolveSearch!: (val: {
   ...
   */
  'clears active tag filters', async () => {
    mockSearch.mockResolvedValue({
      topics: [makeTopic()],
      posts: [],
    });
   ...
   */
  'changes sort and triggers re-fetch with new sort field', async () => {
    mockSearch.mockResolvedValue({ topics: [], posts: [] });
   ...
   */
  'shows no results message when search returns empty', async () => {
    mockSearch.mockResolvedValue({ topics: [], posts: [] });
   ...
   */
  'applies search term highlighting in results', async () => {
    mockSearch.mockResolvedValue({
   ...
   */
  'performs search automatically when initialQuery is provided', async () => {
    mockSearch.mockResolvedValue({ topics: [], posts: [] });
