...
class SECiDComponents {
  ...
  initNavbar() {
    ...
    if (mobileToggle && mobileMenu) {
      mobileToggle.addEventListener('click', () => {
        const isOpen = mobileMenu.classList.contains(
          'secid-navbar__mobile-menu--open'
        );
        ...
        const openLabel =
          lang === 'es' ? 'Abrir menú de navegación' : 'Open navigation menu';
        const closeLabel =
          lang === 'es' ? 'Cerrar menú de navegación' : 'Close navigation menu';
      ...
      document.addEventListener('keydown', (e) => {
        if (
          e.key === 'Escape' &&
          mobileMenu.classList.contains('secid-navbar__mobile-menu--open')
        ) {
          this.closeMobileMenu(mobileToggle, mobileMenu);
          mobileToggle.focus();
        }
    ...
    navLinks.forEach((link) => {
      link.addEventListener('click', (e) => {
        navLinks.forEach((l) =>
          l.classList.remove('secid-navbar__link--active')
        );
  ...
  initLanguageSelector(navbar) {
    ...
    if (langButton && langDropdown) {
      ...
      document.addEventListener('keydown', (e) => {
        if (
          e.key === 'Escape' &&
          langButton.getAttribute('aria-expanded') === 'true'
        ) {
          langButton.setAttribute('aria-expanded', 'false');
          langDropdown.style.display = 'none';
          langButton.focus();
  ...
  initBottomNav() {
    ...
    const bottomNavItems = bottomNav.querySelectorAll(
      '.secid-bottom-nav__item'
    );
      ...
      if (!item.hasAttribute('data-bottom-nav-more')) {
        item.addEventListener('click', () => {
          ...
          bottomNavItems.forEach((i) =>
            i.classList.remove('secid-bottom-nav__item--active')
          );
  ...
  showBottomNavMoreMenu() {
    ...
    const moreMenuOptions = [
      ...
      {
        label:
          document.documentElement.lang === 'es' ? 'Iniciar Sesión' : 'Login',
        href: '#',
        icon: 'fas fa-sign-in-alt',
      },
      {
        label:
          document.documentElement.lang === 'es' ? 'Registrarse' : 'Register',
        href: `/${document.documentElement.lang}/signup`,
        icon: 'fas fa-user-plus',
      },
      {
        label:
          document.documentElement.lang === 'es' ? 'Configuración' : 'Settings',
        href: '#',
        icon: 'fas fa-cog',
      },
    ];
  ...
  initForms() {
    ...
    forms.forEach((form) => {
      const inputs = form.querySelectorAll(
        '.secid-form__input, .secid-form__textarea, .secid-form__select'
      );
  ...
  validateInput(input) {
    ...
    input.classList.remove(
      ...
      'secid-form__input--success'
    );

    // Remove existing messages
    const existingMessage = input.parentElement.querySelector(
      '.secid-form__error, .secid-form__success'
    );
    if (existingMessage) {
  ...
  makeSwipeable(element, options = {}) {
    ...
    function handleEnd(e) {
      ...
      if (elapsedTime <= config.allowedTime) {
        if (
          Math.abs(distX) >= config.threshold &&
          Math.abs(distY) <= config.restraint
        ) {
          // Horizontal swipe
          ...
          Math.abs(distY) >= config.threshold &&
          Math.abs(distX) <= config.restraint
        ) {
          // Vertical swipe
